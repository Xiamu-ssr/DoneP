...
>
> **省心模式的正文怎么来的**：若你是 Claude / Codex 内核的 agent，Stop hook 会通过 stdin 传一个含 `transcript_path` 的 JSON，donep-notify 会自动读出你**最后一条 assistant 回复全文**作为正文（不截断，手表/手机端自己截断显示）。同时兼容 Claude（`type:assistant` / `content[].text`）与 Codex（`response_item.payload` / `content[].output_text`）两种 transcript 格式。拿不到才降级为项目名 / "完成"。
>
